...
constexpr tInstant CURSORHIDE = INSTANT_SECOND;
...
sRenderer *sdl_renderer_new(sProgram *prog) {
    sRenderer *sdlr = malloc(sizeof(sRenderer));
    assert(sdlr);

    sConfig *const cfg = pg_config(prog);
    ...
    const tUint width = cfg->width;
    const tUint height = cfg->height;
    const tUint scale = cfg->scale;
    ...
    if (pg_config(prog)->refreshmode == CVIS_REFRESHMODE_SYNC) {
        ...
        if (mode && mode->refresh_rate >= 0.0f) {
            const tUint rate = (tUint)mode->refresh_rate;
...
static void update_keyevent(sProgram *p, SDL_KeyboardEvent *e, bool set) {
    switch (e->key) {
    case SDLK_LEFT:
        pg_keymap_set(p, KEYEVENT_LEFT, set);
        break;
    case SDLK_RIGHT:
        pg_keymap_set(p, KEYEVENT_RIGHT, set);
        break;
    case SDLK_UP:
        pg_keymap_set(p, KEYEVENT_UP, set);
        break;
    case SDLK_DOWN:
        pg_keymap_set(p, KEYEVENT_DOWN, set);
        break;
    case SDLK_Z:
        pg_keymap_set(p, KEYEVENT_Z, set);
        break;
    case SDLK_X:
        pg_keymap_set(p, KEYEVENT_X, set);
        break;
    case SDLK_C:
        pg_keymap_set(p, KEYEVENT_C, set);
        break;
    default:
        break;
    }
...
void pg_eventloop_sdl(sProgram *p) {
    ...
    tInstant cursorhide = instant();

    while (running) {
        tInstant now = instant();
...
sUint2d RNDR_SIZE() {
    return (sUint2d){.x = PG_CONFIG()->width, .y = PG_CONFIG()->height};
}
...
void RNDR_FADE(tUbyte a) {
    sRenderer *sdlr = PG_RENDERER();
    sColor newc = sdlr->cfg->background;
    newc.a = a;
    RNDR_COLOR(newc);
...
void RNDR_CLEAR() {
    sRenderer *sdlr = PG_RENDERER();
    sColor c = sdlr->cfg->background;
    SDL_SetRenderDrawColor(sdlr->renderer, c.r, c.g, c.b, c.a);
    SDL_RenderClear(sdlr->renderer);
}

void RNDR_BLEND(SDL_BlendMode eBlendMode) {
    SDL_SetRenderDrawBlendMode(PG_RENDERER()->renderer, eBlendMode);
}
...
void RNDR_AUTORESIZE() {
    ...
    sdlr->cfg->width = uint_min(MAX_LOGICAL_SIZE, (tUint)w / sdlr->cfg->scale);
    sdlr->cfg->height = uint_min(MAX_LOGICAL_SIZE, (tUint)h / sdlr->cfg->scale);
